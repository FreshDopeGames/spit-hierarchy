-- Update clean_official_ranking_items to handle NULL values properly
  ...
  WITH to_delete AS (
    ...
    AND NOT EXISTS (
      ...
      AND (
        ...
        (
          -- Decade filter (group-aware logic with NULL handling)
             ...
             WHERE 
               CASE 
                 ...
                 WHEN EXISTS (SELECT 1 FROM rapper_tag_assignments rta WHERE rta.rapper_id = r.id AND rta.tag_id = group_tag_id) THEN
                   ((r.birth_year IS NOT NULL AND r.birth_year >= (SUBSTRING(decade, 1, 4)::INT) AND 
                     r.birth_year <= (SUBSTRING(decade, 1, 4)::INT + 9))
                   OR
                    (r.career_start_year IS NOT NULL AND r.career_start_year >= (SUBSTRING(decade, 1, 4)::INT) AND 
                     r.career_start_year <= (SUBSTRING(decade, 1, 4)::INT + 9)))
                 -- For solo artists: only check career_start_year
                 ELSE
                   (r.career_start_year IS NOT NULL AND
...
-- Execute cleanup again for Top 80s Rappers
