-- Fix: Update trigger function for attribute votes to only track distinct rappers from votes table
...
BEGIN
  -- Calculate distinct rappers rated (attribute votes only, NOT ranking votes)
...
-- Fix: Update trigger function for ranking votes - only count attribute ratings for rappers_voted_count
...
-- Fix: Backfill rappers_voted_count to only count from votes table (attribute ratings)
...
-- Trigger achievement checks for all users who have attribute votes
DO $$
DECLARE
  user_record RECORD;
BEGIN
