...
const RapperDiscography = ({ rapperId, rapperName = "Unknown Artist", rapperSlug = "", scrollPos }: RapperDiscographyProps) => {
      ...
      <Card className="bg-black border-4 border-[hsl(var(--theme-primary))]">
        <CardContent className="p-6 text-center">
          ...
          <Button
            ...
            variant="outline"
            className="border-[var(--theme-primary)]/50 text-[var(--theme-primary)] hover:bg-[var(--theme-primary)]/10"
  ...
  const mixtapes =
    data?.discography
      ?.filter((item) => item.album?.release_type === "mixtape")
      .sort((a, b) => {
        const dateA = a.album?.release_date ? new Date(a.album.release_date).getTime() : 0;
        const dateB = b.album?.release_date ? new Date(b.album.release_date).getTime() : 0;
        return dateA - dateB; // Chronological order (earliest first)
    ...
    <Card className="bg-black border-4 border-[hsl(var(--theme-primary))] shadow-lg shadow-[var(--theme-primary)]/10 min-h-[600px] sm:min-h-[500px]">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[var(--theme-textMuted)] mt-1">
              {isLoading ? "Fetching from MusicBrainz..." : "Studio releases and underground mixtapes"}
            </p>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-6 sm:p-8 py-[10px]">
        ...
        <div className="flex items-center justify-center gap-2 pt-6 border-t border-[hsl(var(--theme-border))]/20 mt-6 my-[25px] py-[23px]">
          {isAdmin && data?.cached && !data?.rate_limited && (
            <Badge variant="secondary" className="text-xs">
              Cached
            </Badge>
          )}
          {isAdmin && data?.rate_limited && (
          ...
          {isLoading && (
            <Badge
              variant="outline"
              className="text-xs border-[hsl(var(--theme-primary))]/50 text-[hsl(var(--theme-primary))]"
            >
              Loading...
            </Badge>
          )}
