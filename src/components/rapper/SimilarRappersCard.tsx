...
const SimilarRappersCard = ({ rapperId }: SimilarRappersCardProps) => {
  const { similarRappers, isLoading, error } = useSimilarRappers(rapperId);
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {similarRappers.map((rapper) => (
              <SimilarRapperCard key={rapper.id} rapper={rapper} />
            ))}
          </div>
        ) : (
...
const SimilarRapperCard = ({ rapper }: { rapper: any }) => {
  const { data: imageUrl } = useRapperImage(rapper.id, 'medium');
