...
const CareerStatsCard = ({ rapperId }: CareerStatsCardProps) => {
  const { data: stats, isLoading } = useRapperCareerStats(rapperId);
    ...
    <Card className="bg-black border-rap-gold/20 shadow-lg shadow-rap-gold/10">
      <CardContent className="p-6">
        <div className="space-y-4">
          ...
          {stats.careerStartYear && (
            <div className="mt-4 p-3 bg-rap-carbon/30 rounded-lg">
          ...
          {stats.labelAffiliations.length > 0 && (
            <div className="mt-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {stats.labelAffiliations.slice(0, 3).map((affiliation, index) => (
                  <Badge 
                    key={index} 
                    variant="outline" 
                    className="text-xs border-rap-gold/50 text-rap-gold"
                  >
                ...
                {stats.labelAffiliations.length > 3 && (
                  <Badge variant="outline" className="text-xs border-rap-smoke/50 text-rap-smoke">
