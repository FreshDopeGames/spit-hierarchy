...
const StatsOverview = () => {
  const {
    ...
    queryFn: async () => {
      ...
      } = await supabase.from("voting_categories").select("*", {
        count: "exact",
        head: true
      }).eq("active", true);
        ...
        {statCards.map((stat, index) => <Card key={index} className="bg-carbon-fiber border border-rap-gold/40 hover:border-rap-gold/70 transition-all duration-300 hover:transform hover:scale-105 shadow-lg shadow-rap-gold/20 relative overflow-hidden group">
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-sm font-merienda tracking-wide text-rap-platinum">{stat.label}</p>
                  <p className="text-rap-platinum font-merienda font-extrabold text-rap-gold-light">{stat.value}</p>
                </div>
              </div>
            </CardContent>
          </Card>)}
