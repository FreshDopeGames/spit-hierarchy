  ...
  return <div className="space-y-2">
      ...
      <Select value={categoryId} onValueChange={setCategoryId}>
        ...
        <SelectContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100">
          {filteredCategories?.map(category => <SelectItem key={category.id} value={category.id} className="font-kaushan focus:bg-gray-100 dark:focus:bg-gray-700 focus:text-gray-900 dark:focus:text-gray-100">
              {category.name}
            </SelectItem>)}
        </SelectContent>
      </Select>
      {selectedCategoryData && <p className="text-sm font-kaushan text-rap-gold">{selectedCategoryData.description}</p>}
    </div>;
};
