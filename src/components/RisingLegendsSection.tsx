...
const RisingLegendsSection = () => {
    ...
    queryFn: async () => {
      const {
        ...
        error
          ...
          reason,
          rappers (*)
        `).eq("ranking_id", (await supabase.from("official_rankings").select("id").eq("slug", "rising-legends-2024").single()).data?.id).order("position", {
        ascending: true
      }).limit(5);
  ...
  if (isLoading) {
    return <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {Array.from({
          length: 5
        }).map((_, i) => <Card key={i} className="bg-carbon-fiber border-rap-forest/20 animate-pulse">
  ...
  return <div className="mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {validRisingLegends.map(item => <RapperCard key={item.rappers!.id} rapper={item.rappers!} position={item.position} compact={true} />)}
      </div>
