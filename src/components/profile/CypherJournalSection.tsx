...
const CypherJournalSection = ({ userId }: CypherJournalSectionProps) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-card rounded-lg p-6 border-4">
  ...
  if (!data || data.verses.length === 0) {
    return (
      <div className="bg-card rounded-lg p-6 border-4">
        <div className="flex items-center gap-2 mb-4">
          <Mic2 className="h-5 w-5 text-muted-foreground" />
          <h3 className="text-lg font-semibold">Cypher Journal</h3>
        </div>
        ...
        {verses.map((verse) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              {/* Bars count badge */}
              <div className="flex-shrink-0 flex items-center gap-1 px-2 py-1 rounded-full bg-primary/10 text-primary">
