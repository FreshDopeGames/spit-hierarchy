...
const VoteNotesSection = ({
  ...
  return <div className="bg-carbon-fiber/90 border border-rap-gold/30 rounded-lg p-6 shadow-lg shadow-rap-gold/20">
      ...
      {voteNotes && voteNotes.length > 0 ? <div className="space-y-4">
          {voteNotes.map(voteNote => <div key={voteNote.id} className="bg-rap-carbon/30 border border-rap-gold/20 rounded-lg p-4">
              <div className="flex items-start gap-4">
                {voteNote.rappers?.image_url && <img src={voteNote.rappers.image_url} alt={voteNote.rappers.name} className="w-12 h-12 rounded-full object-cover border border-rap-gold/30" />}
          ...
          <p className="text-rap-smoke font-merienda ">No vote notes yet</p>
