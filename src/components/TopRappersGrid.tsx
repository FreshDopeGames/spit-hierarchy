...
const RapperAvatar = ({
  ...
  return <Link to={`/rapper/${rapper.id}`} className="group" onClick={() => window.scrollTo(0, 0)}>
      <div className="flex flex-col items-center space-y-2">
        <div className={`${sizeClasses[size]} rounded-full overflow-hidden bg-gradient-to-br from-rap-carbon to-rap-carbon-light flex items-center justify-center border-2 border-rap-gold/30 group-hover:border-rap-gold transition-colors`}>
          {imageUrl ? <img src={imageUrl} alt={rapper.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" /> : <Music className={`${iconSizeClasses[size]} text-rap-platinum/50`} />}
        </div>
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("rappers").select("*").order("average_rating", {
        ascending: false
      }).order("total_votes", {
        ascending: false
      }).limit(5);
    ...
    return <section className="mb-16">
        <div className="animate-pulse">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {Array.from({
            length: 5
          }).map((_, i) => <div key={i} className="h-64 bg-rap-carbon-light rounded"></div>)}
          </div>
        </div>
      </section>;
  }
      ...
      <Card className="bg-carbon-fiber border-rap-gold/30 shadow-lg shadow-rap-gold/20">
        <CardContent className="p-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            ...
            {showViewAll && <Link to={viewAllLink} className="w-full sm:w-auto" onClick={() => window.scrollTo(0, 0)}>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
              {rappers.slice(0, 2).map((rapper, index) => <div key={rapper.id} className="flex items-center space-x-4 sm:space-x-6 p-6 sm:p-4 bg-gradient-to-r from-rap-carbon-light/40 to-transparent rounded-lg border border-rap-gold/30 min-h-[120px] sm:min-h-[100px]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {rapper.real_name && <p className="text-rap-smoke text-sm sm:text-sm font-kaushan mt-1 truncate">
                        {rapper.real_name}
                      </p>}
                  </div>
                </div>)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              {rappers.slice(2, 5).map((rapper, index) => <div key={rapper.id} className="flex flex-col items-center space-y-2 sm:space-y-3 p-4 sm:p-4 bg-gradient-to-b from-rap-carbon-light/20 to-transparent rounded-lg border border-rap-gold/10 min-h-[100px] sm:min-h-[120px]">
                  ...
                  <div className="text-center min-w-0 w-full">
                    ...
                    {rapper.real_name && <p className="text-rap-smoke text-xs sm:text-xs font-kaushan mt-1 truncate">
                        {rapper.real_name}
                      </p>}
                  </div>
                </div>)}
