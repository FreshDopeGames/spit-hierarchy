...
const AdminRankingsManagement = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("official_rankings").select(`
          *,
  ...
  const deleteRankingMutation = useMutation({
    mutationFn: async (rankingId: string) => {
      const {
        error
      } = await supabase.from("official_rankings").delete().eq("id", rankingId);
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      <Card className="bg-carbon-fiber border border-rap-gold/30">
        ...
        <CardContent>
          {isLoading ? <div className="text-center py-8">
            ...
            </div> : <AdminRankingsTable rankings={rankings || []} onEdit={handleEdit} onDelete={handleDelete} />}
        </CardContent>
      </Card>

      {editingRanking && <AdminRankingDialog ranking={editingRanking} onRankingCreated={handleRankingCreated} />}
    </div>;
};
