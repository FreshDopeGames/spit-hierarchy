...
const BlogManagement = () => {
  ...
  const filteredPosts = posts?.filter(post => post.title.toLowerCase().includes(searchTerm.toLowerCase()) || post.status.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="space-y-6">
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Search posts..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-rap-carbon border-rap-smoke text-rap-platinum" />
                </div>
              </div>
            </CardHeader>
            <CardContent>
              {postsLoading ? <div className="text-rap-smoke">Loading posts...</div> : <div className="space-y-4">
                  {filteredPosts?.map(post => <div key={post.id} className="border border-rap-smoke/30 rounded-lg p-4 hover:border-rap-gold/50 transition-colors">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2 ml-4">
                          <Button size="sm" variant="outline" onClick={() => window.open(`/blog/${post.id}`, '_blank')} className="border-rap-smoke text-rap-smoke hover:border-rap-gold hover:text-rap-gold">
                            <Eye className="w-4 h-4" />
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handleEditPost(post)} className="border-rap-smoke text-rap-smoke hover:border-rap-gold hover:text-rap-gold">
                            <Edit className="w-4 h-4" />
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handleDeletePost(post.id)} className="border-red-500 text-red-500 hover:border-red-400 hover:text-red-400">
          ...
          <Card className="bg-carbon-fiber border border-rap-gold/30">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {categories?.map(category => <div key={category.id} className="border border-rap-smoke/30 rounded-lg p-4 hover:border-rap-gold/50 transition-colors">
                    <div className="flex justify-between items-start">
                      <div>
                      ...
                      <Button size="sm" variant="outline" onClick={() => handleEditCategory(category)} className="border-rap-smoke text-rap-smoke hover:border-rap-gold hover:text-rap-gold">
                        <Edit className="w-4 h-4" />
                      </Button>
                    </div>
                  </div>)}
      ...
      {/* Dialogs */}
      <BlogPostDialog open={showPostDialog} onOpenChange={setShowPostDialog} post={selectedPost} onSuccess={() => {
      queryClient.invalidateQueries({
        queryKey: ['admin-blog-posts']
      });
      ...
      <BlogCategoryDialog open={showCategoryDialog} onOpenChange={setShowCategoryDialog} category={selectedCategory} onSuccess={() => {
      queryClient.invalidateQueries({
        queryKey: ['blog-categories']
      });
