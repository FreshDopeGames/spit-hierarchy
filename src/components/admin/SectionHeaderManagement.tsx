...
const SectionHeaderManagement = () => {
  ...
  const { data: headers, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("section_headers").select("*");
  ...
  return (
    <div className="space-y-6">
      <AdminTabHeader 
        ...
        icon={Settings}
        description="Customize section headers and descriptions across the site"
      />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {isLoading ? (
          <div className="text-center py-8 text-rap-platinum">Loading headers...</div>
        ) : (
          Object.entries(headers || {}).map(([section, header]) => (
            <Card key={section} className="bg-carbon-fiber border border-rap-gold/30">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange(section, "title", e.target.value)
                    }
                    className="bg-rap-carbon-light text-rap-platinum border-rap-gold/50"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      handleEditChange(section, "description", e.target.value)
                    }
                    className="bg-rap-carbon-light text-rap-platinum border-rap-gold/50 resize-none"
                  />
                </div>
                <Button
