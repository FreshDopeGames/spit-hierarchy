...
import { ListMusic, Play, Square, RotateCcw } from "lucide-react";
...
const AlbumTracksBulkFetch = () => {
  ...
  const startBulkFetch = async () => {
    ...
    try {
      ...
      // Get total count of albums needing tracks
      ...
      if (totalToProcess === 0) {
        toast.info("No albums need track fetching");
        setIsRunning(false);
      ...
      while (true) {
        ...
        const { data, error } = await supabase.functions.invoke('bulk-fetch-album-tracks', {
        ...
        if (data.completed || data.results.processedInBatch < batchSize) {
          toast.success("Bulk Track Fetch Complete", {
            description: `Processed ${allResults.processed} albums. ${allResults.successful} successful, ${allResults.failed} failed, ${allResults.skipped} skipped.`,
          });
          break;
        }
  ...
  const stopProcess = () => {
    ...
    toast.info("Process Stopped", {
      description: "Bulk track fetch has been stopped. Partial results are preserved.",
    });
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Bulk Album Tracks Fetch
        </CardTitle>
        <CardDescription>
          Automatically fetch tracklists from MusicBrainz for all albums with missing track data.
          This will enable track voting and complete album details.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {isRunning && (
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span>{totalProcessed} albums processed</span>
            </div>
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <p>Batch size: 5 albums per batch with 2s delay between batches.</p>
          <p>Only albums with MusicBrainz IDs and NULL track_count will be processed.</p>
        </div>
      </CardContent>
    </Card>
  );
...
export default AlbumTracksBulkFetch;
