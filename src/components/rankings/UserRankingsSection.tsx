  ...
  return <div>
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <Button variant={filter === "all" ? "default" : "outline"} onClick={() => setFilter("all")} size="sm" className={filter === "all" ? "bg-rap-gold hover:bg-rap-gold-light text-rap-charcoal font-mogra text-xs sm:text-sm" : "border-rap-gold/30 text-rap-gold hover:bg-rap-gold hover:text-rap-charcoal font-mogra text-xs sm:text-sm"}>
            All
          </Button>
          <Button variant={filter === "popular" ? "default" : "outline"} onClick={() => setFilter("popular")} size="sm" className={filter === "popular" ? "bg-rap-gold hover:bg-rap-gold-light text-rap-charcoal font-mogra text-xs sm:text-sm" : "border-rap-gold/30 text-rap-gold hover:bg-rap-gold hover:text-rap-charcoal font-mogra text-xs sm:text-sm"}>
            <Star className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
            Popular
          </Button>
          {user && <Button variant={filter === "my-rankings" ? "default" : "outline"} onClick={() => setFilter("my-rankings")} size="sm" className={filter === "my-rankings" ? "bg-rap-gold hover:bg-rap-gold-light text-rap-charcoal font-mogra text-xs sm:text-sm" : "border-rap-gold/30 text-rap-gold hover:bg-rap-gold hover:text-rap-charcoal font-mogra text-xs sm:text-sm"}>
              Mine
            </Button>}
          
          {/* Create Ranking Button - moved here from header */}
          {user && <CreateRankingDialog>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 py-0 my-[30px]">
        {transformedRankings.map(ranking => <RankingCard key={ranking.id} {...ranking} onClick={onRankingClick} />)}
      </div>

      {/* Load More Button */}
      {hasNextPage && onLoadMore && <div className="text-center mt-8">
          <Button onClick={onLoadMore} disabled={isLoadingMore} variant="outline" className="border-rap-gold/30 text-rap-gold hover:bg-rap-gold hover:text-rap-carbon font-mogra">
      ...
      {!user && <Card className="bg-carbon-fiber border-rap-gold/20 mt-8">
