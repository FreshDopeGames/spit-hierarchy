...
const RankingCard = ({ ranking, isUserRanking = false }: RankingCardProps) => {
  ...
  const rankingLink =
    isUserRanking && !ranking.isOfficial
      ? `/rankings/community-rankings/${ranking.slug}`
      : `/rankings/official/${ranking.slug}`;

  return (
    <Link to={rankingLink} className="block group" onClick={() => window.scrollTo(0, 0)}>
      <div
        className="flex flex-col min-h-[420px] sm:min-h-[650px] md:min-h-[500px] transition-all duration-300 group-hover:scale-[1.02]"
        style={{
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.boxShadow =
            "var(--theme-element-ranking_card-hover-shadow, 0 10px 25px rgba(212, 175, 55, 0.2))";
        }}
        onMouseLeave={(e) => {
