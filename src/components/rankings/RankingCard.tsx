...
const RankingCard = ({
  ...
  return <Link to={rankingLink} className="block group">
      <Card className="h-full bg-carbon-fiber border-rap-silver/30 hover:border-rap-gold/50 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-rap-gold/20">
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="flex items-center gap-2 flex-wrap">
              {ranking.isOfficial ? <Badge variant="default" className="bg-rap-gold/20 text-rap-gold border-rap-gold/30 font-kaushan">
                  <Crown className="w-3 h-3 mr-1" />
                  Official
                </Badge> : <Badge variant="secondary" className="bg-rap-burgundy/20 text-rap-burgundy border-rap-burgundy/30 font-kaushan">
                  <Users className="w-3 h-3 mr-1" />
                  Community
                </Badge>}
              
              {ranking.category && <Badge variant="outline" className="border-rap-smoke/30 text-rap-smoke font-kaushan text-xs">
                  {ranking.category}
                </Badge>}
            </div>
          </div>
          ...
          {ranking.description && <p className="text-rap-smoke text-sm font-merienda line-clamp-2 leading-relaxed">
              {ranking.description}
            </p>}
        </CardHeader>
        ...
        <CardContent className="pt-0">
          {/* Top Rappers Preview */}
          {ranking.rappers && ranking.rappers.length > 0 && <div className="mb-4">
              <h4 className="text-sm font-semibold text-rap-silver mb-2 font-merienda">
                Top {Math.min(ranking.rappers.length, 3)}:
              </h4>
              <div className="space-y-1">
                {ranking.rappers.slice(0, 3).map((rapper, index) => <div key={index} className="flex items-center gap-2 text-sm">
          ...
          <div className="flex items-center justify-between text-xs text-rap-smoke border-t border-rap-smoke/20 pt-3">
            ...
            <div className="flex items-center gap-2 text-right">
              {!ranking.isOfficial && <div className="flex items-center gap-1">
