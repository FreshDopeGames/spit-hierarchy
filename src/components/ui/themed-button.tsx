...
const themedButtonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "bg-[hsl(var(--theme-element-button-default-bg,var(--theme-primary)))] text-[hsl(var(--theme-element-button-default-color,var(--theme-background)))] hover:bg-[hsl(var(--theme-primaryDark))] font-[var(--theme-font-heading)] border-[var(--theme-element-button-default-border-width,1px)] border-[var(--theme-element-button-default-border-style,solid)] border-[hsl(var(--theme-element-button-default-border-color,var(--theme-primary)))] rounded-[var(--theme-element-button-default-border-radius,8px)] p-[var(--theme-element-button-default-padding,0.5rem_1rem)]",
...
export interface ThemedButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof themedButtonVariants> {
  asChild?: boolean;
}
  ...
  ({ className, variant, size, asChild = false, style, ...props }, ref) => {
    ...
    const elementStyle =
      variant === "gradient"
        ? {
            background:
              "hsl(var(--theme-element-button-gradient-bg, var(--theme-gradient-primary-gradient, linear-gradient(135deg, #D4AF37, #E8C547))))",
            ...style,
          }
        : style;
