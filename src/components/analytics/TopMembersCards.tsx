...
const TopMembersCards = ({ timeRange = "all", countryCode, region }: TopMembersCardsProps) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      console.log(
        `✅ RPC returned ${judgesData?.length || 0} top skill judges (query took ${queryEndTime - queryStartTime}ms)`,
      );
  ...
  const { data: topQuizPlayers, isLoading: loadingQuizPlayers } = useQuery({
    ...
    queryFn: async () => {
      ...
      const memberStats = ((publicStats || []) as Array<{ id: string; quiz_questions_answered: number }>)
        .filter(s => (s.quiz_questions_answered || 0) > 0)
        .sort((a, b) => (b.quiz_questions_answered || 0) - (a.quiz_questions_answered || 0))
        .slice(0, 5);
    ...
    <ThemedCard className="bg-black border-[hsl(var(--theme-primary))] border-4 shadow-lg shadow-[var(--theme-primary)]/20">
      ...
      <ThemedCardContent>
        {isLoading ? (
              ...
              <div key={member.id || member.user_id} className="flex items-center space-x-4">
                ...
                <div className="flex-1 min-w-0">
                  {member.profiles?.username && !member.profiles.username.includes('@') ? (
                    ...
                    <p className="text-[hsl(var(--theme-text))]/60 font-semibold text-base truncate">Unknown User</p>
                  )}
          ...
          <p className="text-[hsl(var(--theme-text))]/60 text-base text-center py-8">No data available</p>
        )}
      </ThemedCardContent>
    </ThemedCard>
  );
