...
const CategoryPerformanceCard = ({ countryCode, region }: CategoryPerformanceCardProps) => {
  const isMobile = useIsMobile();
  const { data: categoryAnalytics, isLoading } = useQuery({
    queryKey: ["category-analytics", countryCode, region],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_category_voting_analytics", {
        p_country_code: countryCode || null,
        p_region: region || null,
      });
      if (error) throw error;
      return data?.filter((category: any) => category.name !== "Overall") || [];
    },
  });

  if (isLoading) {
    return (
      <Card className="bg-[var(--theme-surface)] border-[var(--theme-primary)] border-2 shadow-lg shadow-[var(--theme-primary)]/20 animate-pulse">
              ...
              <div key={category.id} className={`p-3 bg-[var(--theme-background)] border border-[var(--theme-primary)]/20 rounded-lg ${isMobile ? "flex flex-col space-y-3" : "flex items-center justify-between"}`}>
                <div className={`${isMobile ? "w-full" : "flex-1"}`}>
                  <div className={`flex items-center gap-2 ${isMobile ? "justify-center" : ""}`}>
                    ...
                    <h4 className="text-[var(--theme-text)] font-medium font-[var(--theme-font-body)] text-3xl">{category.name}</h4>
                  </div>
                  <p className={`text-[var(--theme-textMuted)] font-[var(--theme-font-body)] text-base ${isMobile ? "text-center" : "text-left"}`}>{category.description}</p>
                </div>
