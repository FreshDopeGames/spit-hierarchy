...
const TopVoters = () => {
  const { data: topVoters, isLoading } = useQuery({
    queryKey: ["top-voters"],
    queryFn: async () => {
      // Get user voting stats using the new secure function
      const { data: votingStats, error: statsError } = await supabase.rpc("get_user_voting_stats");
      ...
      if (votingStats && votingStats.length > 0) {
        ...
        // Merge the data and sort by total votes
        return votingStats
          .map(stat => ({
            ...stat,
  ...
  return (
    <ThemedCard className="bg-rap-carbon border-2 border-rap-gold">
      <ThemedCardHeader>
