...
export const useVSMatchBySlug = (slug: string) => {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<VSMatchWithVoteCounts> => {
      ...
      // Get aggregated vote counts and the current user's own vote
      const [countsResult, ownVoteResult] = await Promise.all([
        supabase.rpc("get_vs_match_vote_counts", { p_match_id: data.id }),
        user
          ? supabase
              .from("vs_match_votes")
              .select("rapper_choice_id")
      ...
      const rapper_1_votes = countFor(data.rapper_1_id);
      const rapper_2_votes = countFor(data.rapper_2_id);
      const total_votes = counts.reduce((sum, c) => sum + Number(c.vote_count), 0);
